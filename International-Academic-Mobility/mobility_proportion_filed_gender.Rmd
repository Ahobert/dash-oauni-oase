---
title: "Mobility per field and gender" 
subtitle: "Proportion of international academic mobility per scientific field and gender of included authors"
...
htmltools::browsable(
  ...
  groupBy = "MAIN_FIELD",
  columns = list(
    MAIN_FIELD = colDef(name="Field"),
    gender=colDef(name="Gender"),
    mobile = colDef(name="Mobile",aggregate = "sum",format = colFormat( separators = TRUE, locales = "en-US")),
    nonmobile = colDef(name="Non-mobile",aggregate = "sum",format = colFormat( separators = TRUE, locales = "en-US")),
    percent_mobile = colDef(name = "Mobility percentage",
      ...
      cell = function(value){
        ...
        },format = colFormat(percent = TRUE, digits = 1, locales = "en-US"))
  )
)
  )
)
...
This table reports some descriptive statistics about the authors included in this study, according to their gender, mobility status, and scientific field. Using the filtering option you can choose to display only the results of a specific scientific field, or an overview over all fields. The drop down menu within the fields shows the results broken down by gender.
